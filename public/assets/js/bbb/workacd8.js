;(function (window, Extensions, History, undefined) {
    ...
    Extensions.WorkProjects = new Class({
        ...
        initialize: function(wrap, options) {
            ...
            this.left       = this.wrap.getElement('a.left');
            this.right      = this.wrap.getElement('a.right');
...
//            this._setArrowPos();
        ...
        _setArrowPos: function() {
            var _this = this,
                initial = true;

            function updateBounds() {
                _this.spy.options.min = (_this.left.getPosition().y  - (window.getSize().y / 2));

                _this.spy.options.max = document.id('footer').getCoordinates().top - (window.getSize().y - 100);
            ...
            function updateTop(val) {
                [_this.left, _this.right].each(function (arrow) {
                    arrow.setStyle('top', val);
                });
            }
            ...
            this.spy = new ScrollSpy({
                ...
                onEnter: function() {
                    ...
                    _this.wrap.addClass('fixed-arrows');
                },
                onLeave: function() {
                    updateTop(window.getScroll().y + _this.left.getSize().y);

                    _this.wrap.removeClass('fixed-arrows');
                },
                onTick: function(arr) {
//                    console.log(arr);
                }
